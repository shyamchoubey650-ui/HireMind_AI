...
//         // new
//         .sgp-avail-label { flex-shrink: 0; }
// .sgp-avail-val { text-align: right; min-width: 0; }
      ...
      <style>{`
        *, *::before, *::after {
        ...
        @media (max-width: 680px) {
          ...
          /* Availability: stack label above value */
          .sgp-avail-panel { padding: 14px 12px !important; }
          .sgp-avail-row {
            flex-direction: column !important;
            align-items: flex-start !important;
            gap: 4px !important;
          }
          .sgp-avail-val { text-align: left !important; }

          /* Existing sets: two-line layout (name on top, meta + actions below) */
          .sgp-set-row-item {
            flex-wrap: wrap !important;
            padding: 10px 12px !important;
            gap: 8px !important;
          }
          .sgp-set-left { flex: 1 1 100% !important; }
          .sgp-set-right {
          ...
          /* Detail modal header: title full-width row, actions (Delete + close) on their own row */
          .sgp-detail-card { padding: 20px 16px 24px !important; border-radius: 18px !important; }
          .sgp-detail-head { flex-wrap: wrap !important; gap: 12px !important; }
          .sgp-detail-head > div:first-child { flex: 1 1 100% !important; }
          .sgp-detail-head-actions {
            flex: 1 1 100% !important;
            justify-content: flex-end !important;
          }
          .sgp-detail-title { font-size: 18px !important; }

          /* Question card top row: index+title stacks full width above the type badge */
          .sgp-q-item-top { flex-wrap: wrap !important; row-gap: 8px !important; }
          .sgp-q-item-top-left { flex: 1 1 100% !important; }
          .sgp-q-type-badge { align-self: flex-start !important; }
          .sgp-q-item-title { font-size: 14px !important; }
        }
      `}</style>
            ...
            {(() => {
              ...
              return Object.entries(breakdownData)
                ...
                .map(([questionType, details]) => {
                  ...
                  return (
                    <div className={`sgp-avail-row ${missing > 0 ? "missing" : "ok"}`} key={questionType}>
                      <span className="sgp-avail-label">{questionType.toUpperCase()}</span>
                      <span className="sgp-avail-val">
                        {available} / {required} available {missing > 0 ? `(Short by ${missing})` : '(Ready)'}
                      </span>
                    </div>
                  );
        ...
        <div className="sgp-detail-modal-overlay" onClick={() => setSelectedSetModal(null)}>
          <div className="sgp-detail-card" onClick={(e) => e.stopPropagation()}>
            <div className="sgp-detail-head">
              ...
              <div className="sgp-detail-head-actions">
                <button
                  type="button"
                  className="sgp-set-delete-btn"
                  style={{ height: '34px', padding: '0 14px' }}
              ...
              <div className="sgp-q-list">
                {activeSetQuestions.map((q, qIdx) => {
                  ...
                  return (
                    <div className="sgp-q-item-card" key={q.id || qIdx}>
                      <div className="sgp-q-item-top">
                        <div className="sgp-q-item-top-left">
                          <span className="sgp-q-item-index">
                            #{qIdx + 1}
                          </span>
                          <span className="sgp-q-item-title">{displayTitle}</span>
                        </div>
